...
Vec3f CModel::getVert(int faceId, int vertInFaceId)
{
	int vertid = faces_[faceId][vertInFaceId];

	return verts_[vertid];
}

Vec3f CModel::getNormal(int nfaceId, int normalInFaceId)
{
	int normalid = VNIds_[nfaceId][normalInFaceId];

	return vertNoraml_[normalid];
}

Vec3f CModel::getTexUV(int uvfaceId, int uvInFaceId)
{
	int uvid = VTIds_[uvfaceId][uvInFaceId];

	return vertTextureCoords_[uvid];
}
